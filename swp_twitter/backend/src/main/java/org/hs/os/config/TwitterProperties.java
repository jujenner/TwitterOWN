package org.hs.os.config;

public class TwitterProperties {

    //TwitterAPI Zugangsdaten
    public static final String CONSUMER_KEY = "rY034BtpD1rtYynWu590187Eo";
    public static final String CONSUMER_SECRET = "EY6K5aEosOxg4EzQOb8S78NsUpEnrSIwtyh4eDmU4CEHXxVgMb";
    public static final String ACCESS_TOKEN = "1318885591132635136-YTe51fTK4fqhHuFS4Is4RwQi0XvhBP";
    public static final String ACCESS_TOKEN_SECRET = "ldFPQpzS5rZl1hNrCqkU9DRVYQVBBeG3fxzDnQ0MsMctX";
}




